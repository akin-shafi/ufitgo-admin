...
import guideData from '@/data/operations-guide.json';
import { Search, ChevronDown, ChevronRight, BookOpen, AlertCircle, PiggyBank, Map, ShieldCheck, Banknote } from 'lucide-react';
...
const OperationsGuide = () => {
  ...
  const [activeCategory, setActiveCategory] = useState(guideData.categories[0].id);
  ...
  const filteredCategories = useMemo(() => {
    if (!searchTerm) return guideData.categories;
    
    const lowerSearch = searchTerm.toLowerCase();
    
    return guideData.categories.map(category => {
  ...
  const displayCategories = searchTerm ? filteredCategories : guideData.categories;
  const currentCategoryData = displayCategories.find(c => c.id === activeCategory) || displayCategories[0];

  return (
    <DashboardLayout title="Platform Operations Guide">
            ...
            <>
              <div className="p-6 border-b border-border bg-bg/20 flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-fg/60 mt-1">Operational protocols and edge case resolutions.</p>
                </div>
              </div>
                      ...
                      {isExpanded && (
                        <div className="p-5 border-t border-border/50 bg-card">
                          ...
                          <div className="bg-amber-500/10 border border-amber-500/20 rounded-lg p-4 flex gap-3">
                            ...
                            <div>
                              <div className="text-sm font-bold text-amber-800 mb-1">Resolution Protocol</div>
