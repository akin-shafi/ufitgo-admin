...
import travelFxData from '@/data/travel-fx-architecture.json';
import { Search, ChevronDown, ChevronRight, BookOpen, Layers, Plug, Clock, Code, AlertCircle, Server, Globe, Activity } from 'lucide-react';
...
const TravelFxArchitectureScreen = () => {
  const [searchTerm, setSearchTerm] = useState('');
  const [activeCategory, setActiveCategory] = useState(travelFxData.categories[0].id);
  ...
  const filteredCategories = useMemo(() => {
    if (!searchTerm) return travelFxData.categories;
    
    const lowerSearch = searchTerm.toLowerCase();
    
    return travelFxData.categories.map(category => {
  ...
  const displayCategories = searchTerm ? filteredCategories : travelFxData.categories;
  const currentCategoryData = displayCategories.find(c => c.id === activeCategory) || displayCategories[0];

  return (
    <DashboardLayout title="Travel FX Finder Reference">
...
export default TravelFxArchitectureScreen;
